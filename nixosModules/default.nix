{ lib }:
let
  inherit (builtins) filter map readDir toString attrNames listToAttrs;
  ...
  generateModulesAuto = root:
    let
      moduleFolderNames = attrNames (filterAttrs (_n: v: v == "directory") (readDir (toString root)));
